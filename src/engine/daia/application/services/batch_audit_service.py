...
class BatchAuditService:
    ...
    ) -> AuditResult | Tuple[AuditResult, Dict[str, Any]]:
        """
        Procesa un audio individual y devuelve el agregado de dominio.
        Puede opcionalmente devolver el resultado raw del pipeline para reporting.
        """
        audit_result, raw_result = self._process_audio_path(Path(audio_path), service_level)
        return (audit_result, raw_result) if include_raw else audit_result
    ...
    def process_folder(
        ...
        service_level: str = "standard",
        include_raw: bool = False
    ) -> BatchAuditResult | Tuple[BatchAuditResult, List[Dict[str, Any]]]:
        """
        ...
        audio_extensions = self.orchestrator.config.get(
        ...
        for index, audio_file in enumerate(audio_files, 1):
            logger.info(f"[{index}/{len(audio_files)}] Procesando: {audio_file.name}")
            try:
                audit_result, raw_result = self._process_audio_path(audio_file, service_level)
                results.append(audit_result)
                raw_results.append(raw_result)

                status_icon = "✅" if audit_result.is_passing else "⚠️"
                logger.info(
                    "  %s QA: %.1f%% | Findings: %s | Status: %s",
                    status_icon,
                    audit_result.qa_score or 0,
                    audit_result.total_findings,
                    audit_result.overall_status,
                )
            except Exception as exc:  # noqa: BLE001
                logger.error("  ❌ Error procesando %s: %s", audio_file.name, exc)

        batch_result = self._build_batch_result(results, start_time)
        if include_raw:
            return batch_result, raw_results
        return batch_result
    ...
    def _process_audio_path(
        ...
        """
        Ejecuta el pipeline para un audio y retorna el agregado de dominio y el resultado raw.
        """
        ...
        audit_result = self._convert_to_domain_model(raw_result, audio_path, service_level)
        return audit_result, raw_result

    @staticmethod
    def _build_batch_result(results: List[AuditResult], started_at: datetime) -> BatchAuditResult:
        total = len(results)
    ...
    def _convert_to_domain_model(
        ...
        return create_completed_result(
            ...
            processing_time_seconds=raw_result.get(
                'processing_time_seconds',
                raw_result.get('processing_time', 0)
            )
        )
